...
class Core:
    def __init__(
        ...
        self.callback_button_handlers = {}  # обработчики callback кнопок
        ...
        # обработчик функции, которая вызывается после отмены ввода
    ...
    def edit_message(self, text, keyboard={}, parse_mode='markdown'):
        ...
        response = requests.get(
            f'{self.url}/editMessageText', data=message_data)
        if response.json()['ok'] is False:
            logger.error(
                f'telegram response for edit_message: {response.json()}')
    ...
    def send_message(self, text, chat_id=None, keyboard={}, parse_mode='markdown'):
        ...
        if response.json()['ok'] is False:
            logger.error(
                f'telegram response for send_message: {response.json()}')
    ...
    def send_photo(self, photo, chat_id=None, caption='', keyboard={}, parse_mode='markdown'):
        ...
        if response.json()['ok'] is False:
            logger.error(
                f'telegram response for send_photo: {response.json()}')
    ...
    def send_document(self, document, chat_id=None, caption='', keyboard={}, parse_mode='markdown'):
        ...
        response = requests.get(
            f'{self.url}/sendDocument', data=message_data, files=document_data)
        if response.json()['ok'] is False:
            logger.error(
                f'telegram response for send_document: {response.json()}')
    ...
    def download_file(self, file_id, path=None):
        ...
        if response.json()['ok'] is False:
            logger.error(
                f'telegram response for download_file: {response.json()}')
            return

        telegram_file_path = response.json()['result']['file_path']
        file = requests.get(
            f'https://api.telegram.org/file/bot{self.token}/{telegram_file_path}')
    ...
    def bind_callback(self, callback_data, handler, data=None):
        ...
        data : |str|, optional
            Data to be passed to the function.
        '''
        self.callback_button_handlers[callback_data] = {
